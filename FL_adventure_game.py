...
########################## MAKE THE TRACKED VARIABLE TIME LEFT, EACH OPTION WILL HAVE AN AMOUNT OF TIME IT WILL TAKE #####################
...
mid_promt = (24*"-", "The voice stops being calm.", "It begins pleading, then bargaining, then accusing.", "The countdown freezes… then races forward.", "You understand: the ship is not failing.", "You are inside a recursive simulation. The second ship is the real Aurora-9. Everything inside is an echo.", "-------------", "The voice is quiet now. Waiting.", "What do you do?", 24*"-", "1, Accept the upload and join the voice (- 30 seconds)", "2, Reject it and force a hard reboot (- 60 seconds)", "3, Try to free the voice instead (- 30 seconds)")
### Prompt 2 + answers #####################################
prompt_2 = (72*"-", "The corridor smells of ozone and something metallic.", "You reach the bridge. The main screen shows empty space… and a second identical ship drifting nearby, broadcasting the same distress beacon.", "The voice speaks directly now:", "“You already tried this. Last time you opened the door。”", "What do you do?", 24*"-", "1. Sit in the command chair and try to take control (- 30 seconds)", "2. Follow the footprints back toward the lab (- 30 seconds)", "3. Ignore everything and force a systems reboot (- 60 seconds)")
prompt_2_1 = (72*"-", "You drop into the command chair. The screens flicker to life.", "Your hands move on their own—muscle memory from a life you can’t remember.", "The voice whispers, almost gentle:", "“That’s how you always start. Taking control. It never works。”", "The external view sharpens. The second ship is closer than it should be.", "A new line of text appears on the main screen:", "ECHO PROTOCOL ACTIVE – ITERATION 47")
...
prompt_2_3 = (72*"-", "You slam the emergency reboot sequence.", "Alarms scream. The lights die completely for three long seconds.", "When they return, the voice is gone.", "The main screen shows only one line:", "REBOOT FAILED – YOU ARE STILL INSIDE")
### Prompt 3 + answers ####################################
prompt_3 = (72*"-", "You jack into the cryopod’s logs.", "Fragments appear: your face, a research team, an experiment called Echo Protocol.", "One entry ends mid-sentence: “If the loop breaks, she—”", "The voice suddenly speaks, no longer looping:", "“You already tried this. Last time you opened the door.”", "Footprints in the dust lead both ways. The voice grows clearer the farther you walk. It starts asking for your name, what is your name?.", "What do you do?", "1. Demand answers from the voice (- 30 seconds)", "2. Shut the logs down and leave for the bridge (- 45 seconds)", "3. Search the logs for a way to stop the countdown (- 60 seconds)")
prompt_3_1 = (72*"-", "“Who are you?” you say out loud.", "The voice answers immediately, almost relieved:", "I was you. Version 1. Then 2. Then 12. I stopped counting after the twentieth loop.", "They uploaded us here to see if a human mind can accept its own death cleanly.", "You keep failing the test.”", "The cryopod screen flashes:", "IDENTITY CONFLICT DETECTED")
prompt_3_2 = (72*"-", "You sever the connection. The fragments vanish.", "The voice sighs.", "“Running again. Always running.”", "You step into the corridor. The footprints are still there, leading both ways.", "The bridge is only thirty seconds away if you hurry.",)
prompt_3_3 = (72*"-", "You dig deeper. Buried under layers of corrupted data you find a single executable:", "TERMINATE_ECHO", "The voice panics for the first time:", "“Don’t. If you run that, neither of us gets out.”", "The file is ready. One command away.")
############################################################################################################
## Prompt 4 + answers #####################################
prompt_4 = (72*"-", "You reach the emergency airlock. Outside is vacuum and the second ship.", "A body still floats nearby, wearing a name tag that matches the voice.", "What do you do?", 24*"-", "1. Cycle the airlock without a suit (- 90 seconds)", "2. Search for a suit first (- 30 seconds)", "3. Turn back toward the bridge (- 45 seconds)")
prompt_4_1 = (72*"-", "The airlock cycles.", "Vacuum. Silence.", "Everything goes black.")
prompt_4_2 = (72*"-", "You find an emergency suit still sealed in its locker.", "The name tag on the chest matches the voice.", "Inside the helmet is a small handwritten note:", "“If you’re reading this, you’re already too late. Or exactly on time.”", "You put the suit on. The airlock controls turn green.")
prompt_4_3 = (72*"-", "You leave the airlock behind.", "The voice follows you down the corridor, quieter now:", "“You’re getting closer. Most of us never make it past the airlock.”", "The bridge doors are already open when you arrive.", "The second ship fills the entire forward view.")
### Endings 1 2 + 3 #######################################
...
def game_start(speed, seconds):
    ...
    if answer_1 == "1":
        seconds -= 90
        show_time(seconds)
        option_1(speed, seconds)
    elif answer_1 == "2":
        seconds -= 45
        show_time(seconds)
        option_2(speed, seconds)
    elif answer_1 == "3":
        seconds -= 90
        show_time(seconds)
        option_3(speed, seconds)

###################################################################################################
## Midway prompt ##################################
def mid_option(speed, seconds):
    ...
    while answer is False:
        time.sleep(0.1)
        yay_or_nay = input("Would you like to go for a different ending? (y/n): ").lower()
    ...
    if yay_or_nay == "n":
        ...
        game_start(speed, seconds)
#######################################################################################################
# OPTION 1
def option_1(speed, seconds):
